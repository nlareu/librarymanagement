...
export function BorrowModal({ asset, users, isOpen, onClose, onConfirm }: BorrowModalProps) {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`${messages.titlePrefix}${asset.title}`}>
            <form onSubmit={handleSubmit}>
                <div className="form-group">
                    ...
                    {users.length > 0 ? (
                        <select
                            ...
                            autoFocus
                        >
                            <option value="" disabled>{messages.selectUserPlaceholder}</option>
                ...
                <div className="modal-actions">
                    ...
                    <button type="submit" className="btn btn-primary" disabled={users.length === 0 || !borrowerId}>{messages.confirmAction}</button>
                </div>
            </form>
        </Modal>
    );
